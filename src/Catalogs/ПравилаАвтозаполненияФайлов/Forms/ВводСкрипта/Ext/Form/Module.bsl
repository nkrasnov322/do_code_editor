...
Процедура РК_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
...
Процедура РК_ПередЗаписьюПосле(Отказ, ПараметрыЗаписи)
	Если Не РК_Клиент.ЕстьПолеHTML(ЭтотОбъект) Тогда
		Возврат;
	КонецЕсли;	
	ЭтотОбъект[ЭтотОбъект.РК_РеквизитКода] = View().getText();
КонецПроцедуры
...
Процедура ЗагрузитьКод() Экспорт
	вью = View();
	вью.minimap(Ложь);
	вью.setText(ЭтотОбъект[ЭтотОбъект.РК_РеквизитКода], Неопределено, Ложь);
	вью.setOriginalText(ЭтотОбъект[ЭтотОбъект.РК_РеквизитКода]);
КонецПроцедуры

&НаСервере
Функция ТекстОписанияПользовательскихОбъектов()
	customObjects = Новый Структура;
	customObjects.Вставить("РезультатОбработки", Новый Структура);
	ТипыОбъектов = Перечисления.ТипыОбъектов;
	СписокРеквизитов = Новый Массив;
	МетаданныеТипаОбъекта = Метаданные.Справочники.Файлы;
	...
	customObjects.Вставить("Файл", НовыйОбъект);
...
&НаКлиенте
Процедура УстановитьКодИзТекста() Экспорт
	View().setContent(ЭтотОбъект[ЭтотОбъект.РК_РеквизитКода]);
КонецПроцедуры
...
Процедура ОбработкаСобытияПолученияПеременной(Событие) Экспорт
	...
	Описание = РК_Сервер.ПолучитьОписаниеПеременной(ИдентификаторПеременной, ИмяПеременной, ПутьКДанным, ЭтотОбъект.РК_АдресХраненияПеременных);
...
&НаКлиенте
Процедура РК_ДобавитьСклонениеПосле(Команда)

	Если Не РК_Клиент.ЕстьПолеHTML(ЭтотОбъект) Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьКодИзТекста();
КонецПроцедуры

&НаКлиенте
&После("ДобавитьАлгоритмЗавершение")
Процедура РК_ДобавитьАлгоритмЗавершение(Результат, Параметры)
